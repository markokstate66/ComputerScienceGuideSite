...
### Added
- `docs/FINAL_GATE.md`: whole-site AdSense-readiness audit (crawl of the build + independent critic, no blocking findings), run-code 67/67, verify-page results; blind A/B judging split into #141, performance follow-up #142 (#143).
- Wave 3 integration: recorded cross-links wired into 44 articles; every hub has its own intro and a "Why here" note per article; three learning paths on /start-here/; 22 new glossary terms; homepage lede covers all ten topics; `tools/click-depth.mjs` (#140).
- Google AdSense loader and region-scoped consent-mode defaults (Privacy & messaging CMP in the EEA/UK/CH, GPC honoured); Privacy Policy, Terms, About and Editorial Policy disclose advertising; byline is the publication (Organization in JSON-LD); robots.txt blocks training-only AI crawlers and allows search/answer engines; max-image-preview:large; og:locale en_US (#139).
