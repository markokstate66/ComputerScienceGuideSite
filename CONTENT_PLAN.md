...
| ` ```csharp run fails ` | Must exit non-zero (normally a failure) — for a file-based **xUnit v3** test program (`#:package xunit.v3@1.*` at the top of the block, no `dotnet test`/project needed) whose deliberately-red test makes it exit 1, e.g. a TDD red step. Without `fails`, non-zero exit is always a failure. The `text output` block after it is the exact console summary xUnit prints (wildcard the version banner and the `Time:` value with `[...]`; the run assembly name is the block's `id`, so it is deterministic). |
