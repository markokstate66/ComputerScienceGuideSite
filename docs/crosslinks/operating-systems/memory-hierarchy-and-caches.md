...
- "a separate article's subject in its own right" | /csharp-dotnet/span-and-memory/ | "Struct-of-arrays: paying only for the fields the loop reads", last sentence of the paragraph that ends "goes one step further and stops paying for fields a given loop never reads at all."
