...
|---|---|---|---|
| 1 | **Author display name** exactly as you want it on bylines | `src/content/authors/`, every article byline, Article JSON-LD | open — byline currently attributes to the publication name ("Computer Science Guide") instead of showing broken placeholder text |
| 2 | **Author bio facts**: current role, what you actually work on, which technologies, anything you want to link (GitHub, LinkedIn). Only true statements; no "years of experience" unless you give the number | About page, author box | open — bio paragraph is omitted (not shown) until this is answered |
...
| 9 | **Consent management (CMP)** for EEA/UK/Swiss ad personalisation | Privacy Policy | resolved (provisional) — owner will choose once AdSense accepts the site; page now says no CMP is active because no advertising is active, and will be added (with the specific platform named) before any ad code goes live |
