...
5. **Consumer verifies** with `node tools/gate.mjs <pillar>/<slug>` from PowerShell: check-article + run-code + verify-page in one run, combined evidence in `.verify/<pillar>__<slug>/gate.json`. Its browser pass takes a machine-wide lock, so parallel runs queue instead of skewing Lighthouse.
...
7. **Record.** Commit the article, its crosslinks file and the three review files. Push the branch. Comment on the issue with a one-line score table for the round and the top open issues.
   - **Passed:** keep `draft: true` (the owner's `/ship` sets `draft: false`; nothing is published before the owner approves it), set `published`/`updated` to today's date, commit, open the PR (`Closes #<n>`) into the integration branch, move the issue to `review`.
