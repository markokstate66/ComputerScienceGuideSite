...
## TCP and HTTP first, without TLS
...
Task server = Task.Run(async () =>
{
    ...
    while (!seen.EndsWith(
        blankLine, StringComparison.Ordinal))
    {
        int n = await stream.ReadAsync(
            buffer);
        if (n == 0) break;
        seen += Encoding.ASCII.GetString(
            buffer, 0, n);
    }
