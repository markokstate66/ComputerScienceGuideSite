...
The original "Note: reread Dune" and "Note: borrowed Hyperion" did not stop existing. `annotate` (not `annotate-for-rebase`) still names them, which is the only reason they are still reachable; a rebase that was not shadowed by a second, untouched branch name would leave them dangling in exactly the sense the [prerequisite on Git internals](/version-control/how-git-works/#a-branch-is-a-41-byte-file-and-a-log-line) describes for any commit nothing points at. Recovering from a rebase you did not mean to run, before `git gc` sweeps those dangling commits away, [is the reflog's job, and it deserves its own worked example rather than a paragraph here](/version-control/undoing-things-in-git/).
