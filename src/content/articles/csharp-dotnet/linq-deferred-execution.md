...
Swap `MyWhere`/`MySelect` for `Where`/`Select` and only the labels change; every `source:`, `testing`, and `projecting` line lands in the same place relative to the others. Behaviorally, `MyWhere`/`MySelect` and the real operators are equivalent: same deferral, same streaming, same trace order. Under the hood they differ. The real `Enumerable.Where` and `Enumerable.Select` are more careful about edge cases (null checks, an indexed overload, a fast path when the source is an array or a `List<T>`), and that care is why their main overloads don't compile down to a `yield return` state machine at all: they dispatch to hand-written iterator classes — one for a plain `IEnumerable<T>`, another for an array, another for a `List<T>` — each with its own explicit, hand-coded `MoveNext()`, written by hand specifically so the array and list cases can skip work a generic `foreach` can't. Only the indexed overloads (`Where` and `Select` with a `(T, int)` selector) fall back to a `yield return` iterator, the same technique `MyWhere` and `MySelect` use throughout. Same contract as your hand-rolled version, different implementation strategy — the point of writing your own was never to match the internals, only to prove the behavior.
